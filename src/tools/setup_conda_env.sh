...
install_dependencies() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        if grep -q "dev =" pyproject.toml; then
             ...
             python -m pip install "ruff>=0.16.0" "mypy>=1.8.0" "ty" "pyrefly" "vulture>=2.16" "skylos>=4.27.0" "pytest>=8.0.0" "pytest-cov>=4.0.0" "pydantic>=2.0.0" "types-defusedxml" "pre-commit"
        fi
