...
- The `tools/update.sh` helper supports environment flags: `INSTALL_TORCH=1` for PyTorch, `MLX_METAL_JIT=OFF` to disable JIT compilation (default: `ON` for local MLX builds), `CLEAN_BUILD=1` to clean build artifacts first.
