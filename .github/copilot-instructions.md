...
| File | Purpose | Size |
| ------ | --------- | ------ |
| `src/check_models.py` | **Single-file CLI monolith** (~24700 lines). All logic lives here. | ★ primary edit target |
...
- **Validation artifact hygiene**: Validation tests must not rewrite tracked `src/output/` assets or leave generated files (reports, wheels, `build/`, `*.egg-info`, temporary configs) anywhere under `src/`. Route every generated output to a temp directory (`tmp_path`). Ordinary gitignored tool caches (`__pycache__`, `.pytest_cache`, `.ruff_cache`, `.mypy_cache`, `.skylos`) are not modifications and may stay where their tools put them.
...
| `hf-cli` | Hugging Face Hub CLI (`hf`) reference for cache, download, repo and hub operations; generated by `hf skills add` (regenerate with `--force`, do not hand-edit; excluded from markdownlint) | `.agents/skills/hf-cli/SKILL.md` |
